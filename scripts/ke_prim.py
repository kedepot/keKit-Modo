# python#
# ke_prim v2.3 - Kjell Emanuelsson 2018-21
#               2.3 - Python3
#               2.2 - Made cube & cylinder unit be "grounded" instead of centered. Use "noground" argument to center,
# 				      as before.
#				2.1 - Added "unit primitive at origo" function. Similar to ctrl-click on primitive tool in the menu,
#	         		  excepts it does not reset all your current values. Also actually unit sized.
#
...
#				Mouse hover w. nothing selected = Aligns workplane to element under mouse and fires set primitive tool.
#
# Usage : 		Run in command line with "@ke_prim.py", assign to hotkey or pie menu. With optional arguments below (
# with space betweeen). E.g: "ke_prim.py cylinder"
#
...
#				unit		 : Creates a unit primitive at 0,0,0 - without resetting segments/sides etc.
#	               			  (cylinder, sphere and cone for now. The rest will reset like default ctrl click)
#				noground     : Wont "ground" cube & cylinder using the unit argument.
#
...
if unit and set_prim == "prim.cube":   # coz cannot nest the cube sizes for some reason
    ...
    if not noground:
        lx.eval('tool.attr prim.cube cenY 0.5')
    lx.eval('tool.apply')
    ...
    if "prim.sphere" or "prim.cylinder" or "prim.cone" in set_prim:
        ...
        if set_prim == "prim.sphere":
            lx.eval('tool.attr "%s" cenY 0' % set_prim)
        elif not noground:
            lx.eval('tool.attr "%s" cenY 0.5' % set_prim)
