# python
# QuickTube v1.5 for Modo - Kjell Emanuelsson 2018-21
# A quickpipe (insp by seneca menards perlscript) script
# Tested on Modo v12
# Contact: sendtonulldevice@gmail.com
# 1.5 - Python3 fixes
# 1.4 - Kit version. Closes loops (edges).
# 1.3 - Fixed some more bugs (tested on 11.2)
# 1.2 - Fixed a few bugs
# 1.1 - Offset now compensates for low side count subdivision tubes = tighter fit! (might be a micron off,
# but not even a problem! (thx Auvinen & Ålind! )
#
# Makes pipes along element selections (using tube tool): 
#					VERTS (in selected order), EDGES (in any order - crossing loops will cause random directions),
#					and POLYGON faces (makes a border edge selection)
...
# 
# Installation: Place .py file in "scripts" folder for Modo
#               (For PC usually C:\Users\ *insert name here* \AppData\Roaming\Luxology\Scripts )
# 				optional pie form example: Place .cfg file in "configs" folder for Modo
# 				(For PC usually C:\Users\ *insert name here* \AppData\Roaming\Luxology\Configs )
#
...
#				  "offset" number must be first in order (if used) and then the "user" values
#				  (in the order: sides, segments, radius)
#
# Arguments:	(no argument)	:	makes tubes on selections in face poly mode but also closes tubes
#                                   that are continous loops (no caps, ever, sorry)
#				offset			:	offsets the tube by tube tool radius - makes the tube be placed ON the edge or vert
#	                    			(instead of in center)
#									note: based on radius = low side count will make offset gap (when subd'd)
#									more sides = better offset
#				offset 0.01 	:	(0.01 is an example value) offset with value overrides the tube tool radius
...
#				automeshtype	:	checks the connected mesh for poly type and apples the same : face, subd or psubd
#				user 4 1 0.01	:	(example values) specifies SIDES, SEGMENTS and RADIUS overrides. 
#									It will check the tube tool and leave those values the same,
#									enabling you to switch from preset tube hotkeys/pie to tube tool defined tubes)
#									*ALL three values must always be entered when using the 'user' argument.*
#									You can combine with offset (+offset value for additional offset distance.)
#				scale			:	Allows you to immediately scale the tubes after operation
#	                    			(Selects tube(s) rings and sets appropriate AC)
#									*Will not close tubes*
#									The mesh can still also be selected in polygon mode with 'select' argument.				
#
# Recommended use: set up your desired use cases in a pie menu (or whatever form)
# and also hotkey the most frequently used one(s)
...
onTop = False
edgeSel = []
ograd, ogseg, ogside, ogtwist = 0, 0, 0, 0
...
if usrval_sel == "Select":
    select = True
elif usrval_sel == "Scaling":
    scale = True
else:
    pass
...
def fn_pipeTool():
    ...
    for i in rowList:
        ...
        tubeSegNr = 1
        for v in i:
            pos = lx.eval('query layerservice vert.wdefpos ? %s' % v)
            if offset:
...
if meshType != "face":
    ...
    if not select and not scale:
        lx.eval('select.drop polygon')
    if not select and automeshtype and not scale:
        lx.eval('select.drop polygon')
