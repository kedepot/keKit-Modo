...
# ke_fitprim 3.0 - Kjell Emanuelsson 2019-2023
#
# 3.0 :  Reverted to previous version due to issues with filter()
# 2.6 :  Fixed two-vert mode offset issue with two selected meshes with item transform.
# 2.5 :  Actually Fixed single poly fit issue
# 2.44 : Fixed single poly fit issue
...
def fn_convert_unit(sizevalue):	
	...
	if check_unit != "meters" :
		...
		elif check_unit == "centimeters" : sfactor = 100
...
for layer in layers :
	...
	if sel_mode == "vertex" :
		...
		vp = []
		for i in v :
			vp.append(lx.eval('query layerservice vert.wdefpos ? %s' %i ))
		vert_pos.append(vp)
		...
		if layer == sel_layer : # get size from initial selection shortest edge
			...
			ev1 = lx.eval('query layerservice vert.wdefpos ? %s' % selected_verts[0][0])
			el = []
			for v in vertlist:
				ev2 = lx.eval('query layerservice vert.wdefpos ? %s' % v)
				el.append(fn_dist(ev1, ev2))
			el.sort()
			edgelen = el[0]
...
if is_selection :	
	...
	selected_index_list = list([_f for _f in selected_index_list if _f])
	...
	if sel_mode == "vertex" :
		...
		sel_count_verts = [_f for _f in selected_verts if _f]
		...
		sel_count_edges = [_f for _f in selected_edges if _f]
		...
		sel_count_polys = [_f for _f in selected_polys if _f]
...
if not is_selection :
	...
	if pi_mode == False and len(first_island) <= 5:
	...
	for i in lx.evalN('query layerservice edges ? selected') :
		evs = [int(i) for i in re.findall(r'\d+', i)]
		ev1 = lx.eval('query layerservice vert.wdefpos ? %s' % evs[0])
		ev2 = lx.eval('query layerservice vert.wdefpos ? %s' % evs[1])
		edges.append(fn_dist(ev1, ev2))
	edges.sort()
	# perimeter = edges[0]
	# else :
	...
	if single_poly_fit :
		...
		side_size = sum(edges) / pi
		# side_size = perimeter / pi
	else :
		side_size = sum(edges) / 4
...
print("--------------------------------")
print("Side Size:", side_size, "Disv:", length)
# print is_selection, prim_pos
print("Pi-Mode:", pi_mode)
