...
# ke_xthick 1.1 (extract and thicken) - Kjell Emanuelsson 2017-21
# 1.1 - Python3 fixes
# Description: Extracts selected polygons and thickens them - 
# 				automatically setting the shifted distance to the shortest edge length 
...
#			   NOTE : Symmetry is not supported. it will restore symmetry to avoid dupe polys, 
#			   but selection treated as above (if islands) 
#					  Work plane will be retained also.
# Usage:	   Run in command line with "@ke_xthick.py", assign to hotkey or pie menu. With optional arguments below 
# 				(with space betweeen)
#
...
def fn_crossproduct_coordlist(coordlist):
    ...
    cr = [v1[1] * v2[2] - v1[2] * v2[1], -1 * (v1[0] * v2[2] - v1[2] * v2[0]), v1[0] * v2[1] - v1[1] * v2[0]]
    return cr
...
if not basic:
    ...
    if not single_island_mode:
        ...
        cr = fn_crossproduct_coordlist(point_verts)
        ...
        cr = fn_crossproduct_coordlist(plane_verts)
